...
const REQUIRED_HEADERS = ["상품명", "저자", "출판사", "정산대상판매가총액", "정산액"] as const;

export function parseKyoboXlsxAdapter(
  ...
  if (!firstSheetName) {
    return {
      rows: [],
      issues: [createIssue(context, "Kyobo workbook does not contain a worksheet.")],
    };
  }
  ...
  if (!sheet) {
    return {
      rows: [],
      issues: [createIssue(context, `Kyobo sheet \"${firstSheetName}\" is missing.`)],
    };
  }
  ...
  if (rows.length < DATA_START_ROW_INDEX) {
    return {
      rows: [],
      issues: [createIssue(context, "Kyobo worksheet is missing the contracted data rows.")],
    };
  }
  ...
  if (header.length === 0 || header.every((cell) => cell === "")) {
    return {
      rows: [],
      issues: [createIssue(context, "Kyobo header row is empty.")],
    };
  }
  ...
  if (header.some((cell) => cell === "")) {
    return {
      rows: [],
      issues: [createIssue(context, "Kyobo header contains an empty header key.")],
    };
  }
  ...
  if (duplicateHeader) {
    return {
      rows: [],
      issues: [createIssue(context, `Kyobo header contains a duplicate key: ${duplicateHeader}.`)],
    };
  }
  ...
  if (missingHeader) {
    return {
      rows: [],
      issues: [createIssue(context, `Kyobo contracted header is missing: ${missingHeader}.`)],
    };
  }
...
function readWorkbook(
  ...
  try {
    ...
    return {
      issue: createIssue(context, "Kyobo XLSX adapter expects ArrayBuffer or typed array contents."),
    };
  } catch {
    return {
      issue: createIssue(context, "Kyobo XLSX file could not be parsed."),
    };
  }
}
...
function createIssue(context: FileAdapterContext, message: string): ParseIssue {
  return {
    issueId: `${context.batchId}-${context.platform}-kyobo_xlsx-parse_error-file`,
